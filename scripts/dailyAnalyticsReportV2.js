...
summary AS (
  SELECT
    ...
    COUNT(DISTINCT account_id) FILTER (WHERE event_name = 'mirror_questionnaire_started') AS mirror_questionnaire_starters,
    COUNT(DISTINCT account_id) FILTER (WHERE event_name = 'mirror_questionnaire_completed') AS mirror_questionnaire_completers,
    COUNT(DISTINCT account_id) FILTER (WHERE event_name = 'mirror_completed' AND metadata->>'mirrorNumber' = '1') AS mirror_1_completers,
    COUNT(DISTINCT account_id) FILTER (WHERE event_name = 'mirror_completed' AND metadata->>'mirrorNumber' = '2') AS mirror_2_completers,
    COUNT(DISTINCT account_id) FILTER (WHERE event_name = 'mirror_completed' AND metadata->>'mirrorNumber' = '3') AS mirror_3_completers,
    COUNT(DISTINCT account_id) FILTER (
      WHERE event_name = 'mirror_analysis_read_depth'
        AND CASE WHEN COALESCE(metadata->>'depth', '') ~ '^\\d+
flow_conversion AS (
...
main().catch((error) => {
  ...
  process.exitCode = 1;
}); THEN (metadata->>'depth')::int ELSE 0 END >= 50
