...
export function createAffiliateSubscriptionAttributionService({
    ...
    async function loadAutomaticHandoff(
        ...
        if (cleanInstallationId) {
            const installationResult = await queryAutomaticHandoffs(client,
                ...
                WHERE handoff.installation_id = $1
                  ...
                  AND handoff.last_redemption_started_at IS NOT NULL
                  ...
                  AND handoff.last_redemption_started_at <= $3
                  AND $3 <= handoff.expires_at
                ORDER BY
                    handoff.last_redemption_started_at DESC,
        ...
        // Referral-handoff attribution is always installation-bound. Apple
        // server notifications and account-only reconciliation do not identify
        // the device that chose a creator code, so they can never select a
        // referral handoff by themselves. They wait for client transaction sync
        // from the exact installation instead of guessing from account state.
        return {
            handoff: null,
            ambiguous: false,
