...
const MIN_RETRY_DELAY_MS = 10_000;
...
function retryAfterMilliseconds(response, currentMs) {
    const raw = safeString(response?.headers?.get?.('retry-after'), 256);
    if (!raw) return null;
    ...
    return Math.max(0, parsed - currentMs);
}

function retryDelayMilliseconds({ response, statusCode, attemptIndex, currentMs, random }) {
    const retryAfter = retryAfterMilliseconds(response, currentMs);
    if (retryAfter != null) {
        return Math.max(MIN_RETRY_DELAY_MS, retryAfter);
    }
    ...
    const exponential = MIN_RETRY_DELAY_MS * (2 ** attemptIndex);
        ...
        try {
            ...
            for (let attemptIndex = 0; attemptIndex < MAX_SEND_ATTEMPTS; attemptIndex += 1) {
                ...
                try {
                    ...
                    if (attemptIndex + 1 < MAX_SEND_ATTEMPTS) {
                        ...
                        console.warn(
                            `[FCM] Transport failure for ${cleanToken.slice(0, 8)}...; ` +
                            `retrying once after ${delayMs}ms (${reason})`
                        );
                ...
                if (
                    failure.retryable &&
                    attemptIndex + 1 < MAX_SEND_ATTEMPTS
                ) {
                    const delayMs = retryDelayMilliseconds({
                        response,
                    ...
                    console.warn(
                        `[FCM] Retryable ${failure.code} for ${cleanToken.slice(0, 8)}...; ` +
                        `retrying once after ${delayMs}ms`
                    );
                    await sleepImpl(delayMs);
                    continue;
                }
