const MOBILE_DASHBOARD_CSS = `
    @media (max-width:760px){
      ...
      aside{position:static;height:auto;padding:14px 14px 12px;border-right:0;border-bottom:1px solid var(--line);display:grid;grid-template-columns:minmax(0,1fr) auto;grid-template-areas:"brand lock" "nav nav";gap:12px 10px;align-items:center;background:#0b0d11;}
      .brand{grid-area:brand;padding:0;gap:10px;min-width:0;font-size:18px;line-height:1.15;}
      .brandmark{width:40px;height:40px;border-radius:12px;flex:0 0 auto;font-size:18px;}
...
export function enhanceSubscriptionAdminMobileHtml(html) {
  ...
  output = output
    ...
    .replaceAll(
      ...
      '<link rel="icon" type="image/png" sizes="180x180" href="/subscription-admin-icon.png?v=2" />'
    );
