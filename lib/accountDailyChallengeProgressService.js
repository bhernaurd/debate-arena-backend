...
function normalizeStoredCalendarDate(value, fieldName) {
    if (value instanceof Date) {
        if (Number.isNaN(value.getTime())) {
            fail(
                'daily_challenge_progress_storage_invalid',
                `${fieldName} is invalid in storage.`,
                {
                    status: 503,
                    retryable: true,
                }
    ...
    if (typeof value === 'string') {
        ...
        if (CALENDAR_DATE_RE.test(cleaned)) {
            return cleaned;
        }
    }
...
function normalizeStoredIsoDate(value, fieldName) {
    const date =
...
function normalizeStoredRecord(row) {
    ...
    try {
        ...
        fail(
            'daily_challenge_progress_storage_invalid',
            'Stored Daily Challenge messages are invalid.',
            {
                status: 503,
                retryable: true,
                cause: error,
