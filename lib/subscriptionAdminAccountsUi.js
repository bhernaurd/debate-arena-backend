...
export function enhanceSubscriptionAdminAccountsHtml(html) {
  ...
  output = output.replace(
    ...
    "map.innerHTML='<svg role=\"img\" aria-label=\"First-time App Store downloads by country\" viewBox=\"0 0 1000 500\">'+gridLines+outlines+bubbles+'</svg>'; bindAccountGeoTooltipDismiss(); const rowByCode=new Map(rows.map(row=>[String(row.countryCode||'').toUpperCase(),row])); map.querySelectorAll('.account-map-bubble').forEach(node=>{ const code=String(node.dataset.countryCode||'').toUpperCase(),row=rowByCode.get(code); if(!row)return; node.addEventListener('mouseenter',event=>{ if(!accountGeoTooltipPinned)showAccountGeoTooltip(row,known,event,false); }); node.addEventListener('mousemove',event=>{ if(!accountGeoTooltipPinned)positionAccountGeoTooltip(event); }); node.addEventListener('mouseleave',()=>hideAccountGeoTooltip(false)); node.addEventListener('focus',()=>{ const rect=node.getBoundingClientRect(); if(!accountGeoTooltipPinned)showAccountGeoTooltip(row,known,{clientX:rect.left+rect.width/2,clientY:rect.top+rect.height/2},false); }); node.addEventListener('blur',()=>hideAccountGeoTooltip(false)); node.addEventListener('click',event=>{ event.stopPropagation(); if(accountGeoTooltipPinned&&accountGeoTooltipPinnedCode===code){ hideAccountGeoTooltip(true); return; } showAccountGeoTooltip(row,known,event,true); }); });"
  );
