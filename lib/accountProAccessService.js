...
} from './googlePlaySubscriptionService.js';
...
export function createAccountProAccessService({
    ...
    // Unit-test callers that inject only the legacy App Store repository keep
    // their existing isolated behavior. Production callers provide pool and
    // automatically gain the Google Play entitlement source as well.
    const playService =
        googlePlayService ??
        (pool
            ? createGooglePlaySubscriptionService({
    ...
    async function getCurrentAccess({
        ...
        const appStoreAccess = normalizeAccessRow(
            row,
            cleanAccountId,
            checkedAt
        );
        if (appStoreAccess.hasProAccess) {
            return appStoreAccess;
        }
        ...
        try {
            const playAccess =
                await playService.getCurrentAccess({
                    accountId: cleanAccountId,
                });
            if (playAccess?.hasProAccess === true) {
                return playAccess;
            }
            return Object.freeze({
                hasProAccess: false,
            ...
            if (error instanceof GooglePlaySubscriptionError) {
                fail(
                    'pro_access_unavailable',
                    'Google Play Pro access could not be verified.',
                    {
                        status:
