...
function normalizeAccessRow(
    ...
    if (
        ...
        fail(
            'pro_access_unavailable',
            'The Lifetime Pro entitlement is not active.',
            {
                status: 503,
                retryable: true,
            }
...
export function createPostgresAccountProAccessRepository(
    ...
    return Object.freeze({
        async findCurrentAccess({
            ...
            try {
                const manualGrantResult =
                    await pool.query(
                        `
                        /* account-pro-access:find-current-manual-grant */
                        SELECT
                            manual_grant.account_id,
                            'manual-pro:' || manual_grant.id::text
                            ...
                            manual_grant.granted_at AS last_signed_date
                        FROM account_manual_pro_grants AS manual_grant
                        INNER JOIN accounts AS account
                            ON account.id = manual_grant.account_id
                        WHERE manual_grant.account_id = $1
                          AND manual_grant.revoked_at IS NULL
                          AND (
                                manual_grant.expires_at IS NULL
                                OR manual_grant.expires_at > $3
                          )
                          AND account.status = 'active'
                        ORDER BY
                            manual_grant.granted_at DESC,
                            manual_grant.id ASC
                        LIMIT 1
                        `,
                        [
                ...
                if (manualGrantResult.rows[0]) {
                    return manualGrantResult.rows[0];
                }
            } catch (error) {
                // Migration 036 is additive. During a staged deployment an
                // absent manual-grant table must not regress existing Apple or
                // Google Play entitlement lookup.
                if (error?.code !== '42P01') {
                    throw error;
                }
            }
            ...
            const appStoreResult =
                await pool.query(
                    ...
                    ORDER BY
                        CASE
                            WHEN entitlement.product_id = $4
                             AND entitlement.status = 'active'
                                THEN 0
                            WHEN entitlement.status = 'active'
                                THEN 1
                            WHEN entitlement.status = 'trial'
                                THEN 2
                            WHEN entitlement.status = 'grace_period'
                                THEN 3
                            ELSE 4
                        END,
                        COALESCE(
                        ...
                        entitlement.last_signed_date DESC NULLS LAST,
                        ownership.original_transaction_id ASC
                    LIMIT 1
                    `,
                    [
                        accountId,
            ...
            if (appStoreResult.rows[0]) {
                return appStoreResult.rows[0];
            }
            ...
            try {
                const playResult =
                    await pool.query(
                        ...
                        SELECT
                            account_id,
                            'google-play:' || purchase_token_sha256
                                AS original_transaction_id,
                            ...
                            expires_date,
                            CASE
                                WHEN normalized_status = 'grace_period'
                                    THEN expires_date
                                ELSE NULL
                            END AS grace_period_expires_date,
                            NULL::timestamptz AS revocation_date,
                            last_verified_at AS last_signed_date
                ...
                return playResult.rows[0] ?? null;
            } catch (error) {
                // Production intentionally does not auto-run migrations. During
                // a staged Android rollout, an absent migration 033 must behave
                // exactly like the pre-Android no-Play-entitlement state and
                // must never regress existing App Store/Lifetime access.
                if (error?.code === '42P01') {
                    return null;
                }
