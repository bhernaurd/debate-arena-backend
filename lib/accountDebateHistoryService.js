    ...
    return Object.freeze({
        ...
        async upsertDebate(
            ...
            const deletion = await client.query(
                `
                    /* account-history:check-deletion-tombstone */
                    SELECT deleted_at
                    FROM account_debate_history_deletions
                    WHERE account_id = $1
            ...
            if (deletion.rows[0]) {
                return normalizeRepositoryResult({
                    saved_debate_id: debate.savedDebateId,
            ...
            if (!canonical.rows[0]) {
                const concurrentDeletion = await client.query(
                    `
                        /* account-history:load-concurrent-deletion */
                        SELECT deleted_at
                        FROM account_debate_history_deletions
                        WHERE account_id = $1
                          AND saved_debate_id = $2
        ...
        async deleteDebate(
            ...
            const tombstone = await client.query(
                `
                    /* account-history:create-deletion-tombstone */
                    INSERT INTO account_debate_history_deletions (
                        account_id,
                        saved_debate_id,
                        deleted_at
                    )
                    ...
                    DO UPDATE SET
                        deleted_at = GREATEST(
                            account_debate_history_deletions.deleted_at,
                            EXCLUDED.deleted_at
                        )
                    RETURNING deleted_at
                `,
                [accountId, savedDebateId, deletedAt]
            );

            const removed = await client.query(
                `
                    /* account-history:delete-saved-debate */
                    DELETE FROM account_debate_history
                    WHERE account_id = $1
                      AND saved_debate_id = $2
                `,
                [accountId, savedDebateId]
            );
            ...
            return Object.freeze({
                ...
                deletedAt:
                    tombstone.rows[0]?.deleted_at ?? deletedAt,
            });
        },

        async listDeletedDebateIds({
            accountId,
        }) {
            const result = await pool.query(
                `
                    /* account-history:list-deletion-tombstones */
                    SELECT saved_debate_id
                    FROM account_debate_history_deletions
                    WHERE account_id = $1
                    ORDER BY saved_debate_id ASC
                `,
                [accountId]
            );
            ...
            const result = await pool.query(
                `
                    ...
                    WHERE account_id = $1
                      AND NOT EXISTS (
                            SELECT 1
                            FROM account_debate_history_deletions AS deleted
                            WHERE deleted.account_id =
                                    account_debate_history.account_id
                              AND deleted.saved_debate_id =
                                    account_debate_history.saved_debate_id
                      )
                      AND (
...
export function createAccountDebateHistoryService({
    ...
    if (
        ...
        typeof repo.deleteDebate !== 'function' ||
        typeof repo.listDeletedDebateIds !== 'function'
    ) {
        fail(
            'invalid_debate_history_configuration',
    ...
    async function deleteDebate({
        ...
        const cleanSavedDebateId = requireUuid(
            savedDebateId,
            'savedDebateId'
        );
        const deletedAt = currentServiceDate(now);

        try {
            const result = await repo.withTransaction(
                (client) => repo.deleteDebate(
                    client,
                    {
                        accountId: authorization.accountId,
                        savedDebateId: cleanSavedDebateId,
                        deletedAt,
                    }
                )
            );
            ...
            if (
                !result ||
                result.savedDebateId !== cleanSavedDebateId ||
                typeof result.deleted !== 'boolean' ||
                !(result.deletedAt instanceof Date) &&
                    typeof result.deletedAt !== 'string'
            ) {
                fail(
                    'debate_history_deletion_failed',
                    'The debate-history deletion returned an invalid result.',
                    {
                        status: 503,
                        retryable: true,
                    }
            ...
            fail(
                'debate_history_deletion_unavailable',
                'Debate history could not be deleted.',
                {
                    status: 503,
                    retryable: true,
                    cause: error,
    ...
    async function listDebates({
        ...
        let page;
        let deletedDebateIds = [];

        try {
            if (normalizedCursor == null) {
                deletedDebateIds = await
                    repo.listDeletedDebateIds({
                        accountId: authorization.accountId,
                    });
            }
        ...
        deletedDebateIds = deletedDebateIds.map(
            (savedDebateId, index) =>
                requireUuid(
                    savedDebateId,
                    `deletedDebateIds[${index}]`
                )
        );
        ...
        return Object.freeze({
            ...
            deletedDebateIds:
                Object.freeze(deletedDebateIds),
            nextCursor,
