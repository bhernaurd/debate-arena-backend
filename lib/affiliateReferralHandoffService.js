...
const DEFAULT_REDEMPTION_WINDOW_MS = 60 * 60 * 1000;
...
export function createAffiliateReferralHandoffService({
  ...
  redemptionWindowMs = DEFAULT_REDEMPTION_WINDOW_MS,
} = {}) {
  ...
  const safeRedemptionWindowMs = Number(redemptionWindowMs);
  if (
    !Number.isFinite(safeRedemptionWindowMs) ||
    safeRedemptionWindowMs < 60_000 ||
    safeRedemptionWindowMs > 24 * 60 * 60 * 1000
  ) {
    throw new Error(
      'Affiliate referral redemptionWindowMs must be between one minute and 24 hours.'
    );
  }
  ...
  async function beginRedemption(rawToken) {
    return withTransaction(pool, async (client) => {
      ...
      // The opaque handoff may exist for several days before the customer
      // actually chooses to continue. Once redeem-start occurs, shrink the
      // evidence window to a short Apple-redemption attempt. A later Apple
      // transaction can only use this handoff if its purchase time lands
      // inside this window.
      const currentExpiryMs = new Date(row.expires_at).getTime();
      const redemptionExpiryMs = Math.min(
        currentExpiryMs,
        Date.now() + safeRedemptionWindowMs
      );
      const redemptionExpiresAt = new Date(redemptionExpiryMs);
  ...
  async function claimHandoff({ rawToken, installationId, accessToken = null } = {}) {
    ...
    return withTransaction(pool, async (client) => {
      ...
      // Server-authoritative last-intent-wins rule. A stale client retry must
      // never replace a creator redemption that started later on this same
      // installation. Permanent subscription-chain ownership remains immutable.
      const intentStartedAt =
        row.last_redemption_started_at ||
        row.redemption_started_at;

      const newerIntent = await client.query(
        `
        SELECT id
        FROM affiliate_referral_handoffs
        WHERE installation_id = $1
          AND id <> $2
          AND attributed_original_transaction_id IS NULL
          AND status IN ('redemption_started', 'claimed')
          AND COALESCE(
                last_redemption_started_at,
                redemption_started_at
              ) > $3
        ORDER BY
          COALESCE(
            last_redemption_started_at,
      ...
      if (newerIntent.rowCount > 0) {
        const winnerId = newerIntent.rows[0].id;
      ...
      // This intent is the newest known one, so older still-pending creator
      // paths on the same installation are superseded before it is claimed.
      await client.query(
        `
        UPDATE affiliate_referral_handoffs
        SET
