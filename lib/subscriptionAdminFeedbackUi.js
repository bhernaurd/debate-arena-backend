...
const FEEDBACK_CSS = `
    ...
    .feedback-row-actions { display:flex;align-items:center;justify-content:flex-end;gap:6px;white-space:nowrap; }
    .feedback-trash { width:30px;height:30px;display:inline-flex;align-items:center;justify-content:center;border:1px solid #303641;background:#171a20;color:#8d95a3;border-radius:9px;padding:0;cursor:pointer; }
    .feedback-trash:hover { border-color:#63343a;background:#261216;color:#ff9ca4; }
    .feedback-trash svg { width:14px;height:14px;display:block;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round; }
    ...
    .feedback-detail-actions .feedback-delete { margin-left:auto;border-color:#63343a;background:#261216;color:#ff9ca4; }
    .feedback-detail-actions .feedback-delete:hover { background:#32181c;border-color:#7b4047;color:#ffc1c6; }
...
(() => {
    ...
    return '<table class="feedback-table"><thead><tr><th>Status</th><th>Type</th><th>User</th><th>Feedback</th><th>App</th><th>Submitted</th><th></th></tr></thead><tbody>' +
      items.map((row) => {
        ...
        return '<tr>' +
          ...
          '<td><div class="feedback-row-actions"><button type="button" class="feedback-action" data-feedback-review="' + escFeedback(row.id) + '" data-reviewed="' + (row.reviewedAt ? 'true' : 'false') + '">' + (row.reviewedAt ? 'Mark new' : 'Mark reviewed') + '</button><button type="button" class="feedback-trash" data-feedback-delete="' + escFeedback(row.id) + '" aria-label="Delete feedback" title="Delete feedback"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16"></path><path d="M9 7V4h6v3"></path><path d="M6.5 7l1 13h9l1-13"></path><path d="M10 11v5"></path><path d="M14 11v5"></path></svg></button></div></td>' +
          '</tr>';
      }).join('') +
      '</tbody></table>';
  }
  ...
  function bindRows() {
    ...
    table.querySelectorAll('[data-feedback-delete]').forEach((button) => {
      button.addEventListener('click', async () => {
        ...
        button.disabled = true;
        try {
          await deleteFeedback(button.dataset.feedbackDelete);
          await loadFeedback();
        } catch (error) {
          alert(error.message || error);
          button.disabled = false;
  ...
  async function deleteFeedback(id) {
    return feedbackApi('/feedback/' + encodeURIComponent(id), {
      method: 'DELETE',
    });
  }
  ...
  function openFeedback(id) {
    ...
    drawer.innerHTML =
      ...
      '<div class="feedback-detail-actions"><button type="button" id="feedbackDrawerReview">' + (row.reviewedAt ? 'Mark as new' : 'Mark as reviewed') + '</button><button type="button" id="feedbackDrawerCopy">Copy feedback</button><button type="button" class="feedback-delete" id="feedbackDrawerDelete">Delete feedback</button></div>';
    ...
    document.getElementById('feedbackDrawerDelete')?.addEventListener('click', async (event) => {
      const confirmed = window.confirm(
        'Delete this feedback permanently? This cannot be undone.'
      );
      if (!confirmed) return;
