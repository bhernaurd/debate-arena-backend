export function enhanceSubscriptionAdminOverviewHtml(html) {
  return String(html)
    ...
    .replace(
      ...
      `    <section id="view-breakdown" class="hidden">\n      <div class="grid" id="breakdownMetrics"><div class="loading">Loading breakdown...</div></div>\n      <div class="twocol">\n        <div class="section"><div class="sectionhead"><h2>Access source</h2><span>Production</span></div><div class="dist" id="breakdownSourceDistribution"></div></div>\n        <div class="section"><div class="sectionhead"><h2>Status</h2><span>Production</span></div><div class="dist" id="breakdownStatusDistribution"></div></div>\n      </div>\n    </section>\n\n    <section id="view-customers" class="hidden">`
    )
    .replace(
    ...
    .replace(
      ...
      `state.view=view; qsa('.nav').forEach(n=>n.classList.toggle('active',n.dataset.view===view)); ['overview','breakdown','customers','history','events'].forEach(v=>qs('#view-'+v).classList.toggle('hidden',v!==view));\n    const meta={overview:['Overview','Production subscription health and revenue'],breakdown:['Breakdown','Subscriber mix, access sources and current subscription status'],customers:['Customers','Search and inspect subscription ownership'],history:['History','Revenue, Apple proceeds, trials, conversions and churn over time'],events:['Events','Apple subscription lifecycle activity']}[view]; qs('#pageTitle').textContent=meta[0]; qs('#pageSub').textContent=meta[1];\n    if(view==='overview')loadOverview(); if(view==='breakdown')loadBreakdown(); if(view==='customers')loadCustomers(); if(view==='history')loadHistory(); if(view==='events')loadEvents();`
    );
}
