  ...
  async function exactSubscriberSnapshot(
    ...
    const summaryResult = await pool.query(
      ...
      SELECT
        ...
        COUNT(*) FILTER (WHERE is_promo)::int AS promo_subscribers,
        COUNT(*) FILTER (WHERE is_commission_earning)::int AS commission_earning_subscribers,
        COUNT(*) FILTER (WHERE is_promo_nonrenewal)::int AS promo_nonrenewals,
        COUNT(*) FILTER (WHERE is_canceling)::int AS canceling,
        COUNT(*) FILTER (WHERE status = 'expired')::int AS expired,
        COUNT(*) FILTER (WHERE status = 'billing_retry')::int AS billing_retry,
        ...
        COUNT(*) FILTER (WHERE joined_in_range AND is_promo)::int AS range_promo_subscribers,
        COUNT(*) FILTER (WHERE joined_in_range AND is_commission_earning)::int AS range_commission_earning_subscribers,
        COUNT(*) FILTER (WHERE joined_in_range AND is_promo_nonrenewal)::int AS range_promo_nonrenewals,
        COUNT(*) FILTER (WHERE joined_in_range AND is_canceling)::int AS range_canceling,
        COUNT(*) FILTER (WHERE joined_in_range AND status = 'expired')::int AS range_expired,
        COUNT(*) FILTER (WHERE joined_in_range AND status = 'billing_retry')::int AS range_billing_retry,
    ...
    return {
      ...
      lifetime: {
        totalReferrals,
        activeSubscribers,
        promoSubscribers: Number(row.promo_subscribers || 0),
        commissionEarningSubscribers: Number(row.commission_earning_subscribers || 0),
        promoNonRenewals,
        canceling: Number(row.canceling || 0),
        expired: Number(row.expired || 0),
        billingRetry: Number(row.billing_retry || 0),
        ...
        cancellationRate: percentOf(Number(row.canceling || 0), activeSubscribers),
      },
      range: {
        totalReferrals: rangeReferrals,
        activeSubscribers: rangeActiveSubscribers,
        promoSubscribers: Number(row.range_promo_subscribers || 0),
        commissionEarningSubscribers: Number(row.range_commission_earning_subscribers || 0),
        promoNonRenewals: rangePromoNonRenewals,
        canceling: Number(row.range_canceling || 0),
        expired: Number(row.range_expired || 0),
        billingRetry: Number(row.range_billing_retry || 0),
        ...
        cancellationRate: percentOf(Number(row.range_canceling || 0), rangeActiveSubscribers),
      },
