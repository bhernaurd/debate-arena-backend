// PARTNER_PRICING_V4: exposes active paid subscriber counts by verified Apple transaction price.
import crypto from 'crypto';
import { DateTime } from 'luxon';

import {
  normalizeAppleOfferIdentifier,
} from './affiliateSubscriptionAttributionService.js';
...
export function buildDefaultAppClipCreatorUrl({
  appClipBundleId,
  creatorCode,
} = {}) {
  const cleanBundleId = String(appClipBundleId || '').trim();

  if (!/^[A-Za-z0-9.-]{3,255}$/.test(cleanBundleId)) {
    throw statusError(
      500,
  ...
  const normalizedCode = normalizeAffiliateCode(creatorCode);
  const url = new URL('https://appclip.apple.com/id');
  url.searchParams.set('p', cleanBundleId);
...
export function parseAffiliateDashboardRange(
  range,
  now = DateTime.now().setZone(AFFILIATE_ACCOUNTING_TIMEZONE)
) {
  const rawKey = String(range || 'this_month').trim().toLowerCase();
  const currentMonthStart = now.startOf('month');

  const monthRange = (start) => ({
    key: `month:${start.toFormat('yyyy-LL')}`,
    kind: 'month',
    monthKey: start.toFormat('yyyy-LL'),
    currentMonthKey: currentMonthStart.toFormat('yyyy-LL'),
    start: start.toISODate(),
    endExclusive: start.plus({ months: 1 }).toISODate(),
    label: start.toFormat('LLLL yyyy'),
    isCurrentMonth: start.hasSame(currentMonthStart, 'month'),
  });

  if (rawKey === 'this_month') {
    return monthRange(currentMonthStart);
  }

  // Backward compatibility for old dashboard links and clients. These are no
  // longer presented as buttons in the partner UI, but existing requests keep
  // working while the dashboard moves to explicit calendar-month navigation.
  if (rawKey === 'last_month') {
    return monthRange(currentMonthStart.minus({ months: 1 }));
  }

  const explicitMonth = /^month:(\d{4})-(\d{2})$/.exec(rawKey);
  if (explicitMonth) {
    const year = Number(explicitMonth[1]);
    const month = Number(explicitMonth[2]);
    const start = DateTime.fromObject(
      { year, month, day: 1 },
      { zone: AFFILIATE_ACCOUNTING_TIMEZONE }
    ).startOf('month');

    if (!start.isValid || year < 2000 || start > currentMonthStart) {
      throw statusError(400, 'Invalid dashboard month.', 'invalid_dashboard_month');
    }
...
export function createAffiliateProgramService({
  ...
  referralBaseUrl = 'https://debate-arena-backend-production.up.railway.app',
  publicReferralBaseUrl = process.env.AFFILIATE_PUBLIC_REFERRAL_BASE_URL ||
    'https://theagoraphilosophy.app',
  appClipReferralEnabled = ['1', 'true', 'yes', 'on'].includes(
    String(process.env.AFFILIATE_APP_CLIP_HANDOFF_ENABLED || '')
      .trim()
      .toLowerCase()
  ),
  appClipBundleId = process.env.AFFILIATE_APP_CLIP_BUNDLE_ID || 'com.bhernaurd.TheAgora.Clip',
  ...
  function buildReferralUrl(code) {
    ...
    if (appClipReferralEnabled) {
      // Public creator links stay branded. On supported iPhones the
      // associated-domain App Clip experience intercepts this URL directly.
      // If the browser reaches the web/backend route instead, /r/:code
      // redirects to Apple's default App Clip URL as a seamless fallback.
      return `${String(publicReferralBaseUrl).replace(/\/$/, '')}/r/${encodeURIComponent(normalizedCode)}`;
    }

    return `${String(referralBaseUrl).replace(/\/$/, '')}/r/${encodeURIComponent(normalizedCode)}`;
  }
  ...
  async function createAffiliate(input = {}, actor = 'admin') {
    ...
    const appleOfferIdentifier = input.appleOfferIdentifier == null
      ? null
      : String(input.appleOfferIdentifier).trim() || null;
    const normalizedAppleOfferIdentifier = appleOfferIdentifier
      ? normalizeAppleOfferIdentifier(appleOfferIdentifier)
      : null;
    ...
    if (!isTest && !normalizedAppleOfferIdentifier) {
      throw statusError(
        400,
        'appleOfferIdentifier is required before creating a production affiliate.',
        'affiliate_offer_identifier_required'
      );
    }
    ...
    return withTransaction(pool, async (client) => {
      const affiliateResult = await client.query(
        ...
        VALUES ($1, $2, $3, $3, $4, $5, $6, 'active', $7, $8, $9, $10, $11, $12)
        RETURNING *
        `,
        [
          internalName,
          displayName,
          normalizedCode,
          appleOfferIdentifier,
          normalizedAppleOfferIdentifier,
          affiliateSince,
          codeStatus,
          isTest,
          payoutCurrency,
  ...
  async function listAffiliates() {
    const result = await pool.query(
      ...
      SELECT
        ...
        terms.promo_commissionable,
        COALESCE(subs.total_referrals, 0)::int AS total_referrals,
        COALESCE(subs.current_subscribers, 0)::int AS current_subscribers,
        current_payout.id AS current_payout_id,
        current_payout.payout_period AS current_payout_period,
        current_payout.eligible_revenue::text AS current_month_eligible_revenue,
        current_payout.amount_due::text AS estimated_this_month,
        current_payout.amount_paid::text AS current_month_amount_paid,
        current_payout.status AS current_month_payout_status,
        current_payout.data_status AS current_month_data_status,
        COALESCE(fin.currently_owed, 0)::text AS currently_owed,
        COALESCE(fin.lifetime_commission_earned, 0)::text AS lifetime_commission_earned,
        COALESCE(fin.lifetime_paid, 0)::text AS lifetime_paid,
        COALESCE(alerts.open_alerts, 0)::int AS open_alerts,
        COALESCE(alerts.critical_alerts, 0)::int AS critical_alerts
      FROM affiliates a
      LEFT JOIN LATERAL (
        SELECT
          t.commission_basis,
          t.commission_rate,
          t.promo_commissionable
          ...
          COUNT(*) FILTER (
            WHERE
              ...
              OR (
                ...
                AND e.grace_period_expires_date > NOW()
              )
          )::int AS current_subscribers
        FROM affiliate_subscription_attributions attr
        LEFT JOIN subscription_entitlements e
          ON e.original_transaction_id = attr.original_transaction_id
         AND e.environment = attr.environment
        WHERE attr.affiliate_id = a.id
          AND attr.environment = CASE WHEN a.is_test THEN 'Sandbox' ELSE 'Production' END
      ) subs ON TRUE
      LEFT JOIN LATERAL (
        SELECT p.*
        FROM affiliate_monthly_payouts p
        WHERE p.affiliate_id = a.id
          AND p.environment = CASE WHEN a.is_test THEN 'test' ELSE 'production' END
          AND p.payout_period = date_trunc('month', CURRENT_DATE)::date
        LIMIT 1
      ) current_payout ON TRUE
      LEFT JOIN LATERAL (
        SELECT
          COALESCE(SUM(
            CASE
          ...
          COALESCE(SUM(CASE WHEN p.status <> 'open' THEN p.amount_due ELSE 0 END), 0) AS lifetime_commission_earned,
          COALESCE((
            SELECT SUM(pp.amount)
            FROM affiliate_payout_payments pp
            JOIN affiliate_monthly_payouts paid_period
              ON paid_period.id = pp.monthly_payout_id
            WHERE pp.affiliate_id = a.id
              AND paid_period.environment = CASE WHEN a.is_test THEN 'test' ELSE 'production' END
          ), 0) AS lifetime_paid
        FROM affiliate_monthly_payouts p
        WHERE p.affiliate_id = a.id
          AND p.environment = CASE WHEN a.is_test THEN 'test' ELSE 'production' END
      ...
      LEFT JOIN LATERAL (
        SELECT
          ...
          COUNT(*) FILTER (WHERE al.status = 'open' AND al.severity = 'critical')::int AS critical_alerts
        FROM affiliate_alerts al
    ...
    return result.rows.map((affiliate) => ({
      ...affiliate,
      referral_url: buildReferralUrl(affiliate.normalized_code),
    }));
  }

  async function setAffiliateOperationalStatus({
    affiliateId,
    active,
    actor = 'admin',
  } = {}) {
    ...
    const shouldBeActive = cleanBoolean(active, 'active', false);

    return withTransaction(pool, async (client) => {
      const existingResult = await client.query(
        `SELECT * FROM affiliates WHERE id = $1 LIMIT 1 FOR UPDATE`,
        [affiliateId]
      );
      ...
      if (existing.status === 'archived' || existing.status === 'terminated_for_cause') {
        throw statusError(
          409,
          'Archived or terminated affiliates cannot be reactivated with the operational status control.',
          'affiliate_status_locked'
        );
      }
      ...
      const updatedResult = await client.query(
        ...
        RETURNING *
        `,
        [affiliateId, shouldBeActive ? 'active' : 'inactive']
      );
      ...
      await client.query(
        ...
        VALUES ($1, $2, $3::uuid, 'affiliate', $3::uuid::text, $4::jsonb, $5::jsonb)
        `,
        [
          actor,
          shouldBeActive ? 'affiliate_activated' : 'affiliate_paused',
          affiliateId,
          JSON.stringify({ status: existing.status, codeStatus: existing.code_status }),
          JSON.stringify({ status: updated.status, codeStatus: updated.code_status }),
        ]
      );
  ...
  async function listAffiliateAlerts({ status = 'open', limit = 100 } = {}) {
    const cleanStatus = String(status || 'open').trim().toLowerCase();
    if (!['open', 'resolved', 'all'].includes(cleanStatus)) {
      throw statusError(400, 'Invalid alert status.', 'invalid_affiliate_alert_status');
    }
    const cleanLimit = Math.max(1, Math.min(250, Number(limit) || 100));
    ...
    const result = await pool.query(
      ...
      SELECT
        ...
        a.normalized_code AS affiliate_code,
        a.is_test AS affiliate_is_test
      FROM affiliate_alerts al
      LEFT JOIN affiliates a ON a.id = al.affiliate_id
      WHERE ($1 = 'all' OR al.status = $1)
      ORDER BY
        CASE al.severity
  ...
  async function resolveAffiliateAlert({
    alertId,
    resolutionNote = null,
    actor = 'admin',
  } = {}) {
    if (!UUID_RE.test(String(alertId || ''))) {
      throw statusError(400, 'Invalid alert ID.', 'invalid_affiliate_alert_id');
    }
    const note = cleanText(resolutionNote, 1000);

    return withTransaction(pool, async (client) => {
      const beforeResult = await client.query(
        `SELECT * FROM affiliate_alerts WHERE id = $1 LIMIT 1 FOR UPDATE`,
        [alertId]
      );
      const before = beforeResult.rows[0];
      if (!before) {
        throw statusError(404, 'Affiliate alert not found.', 'affiliate_alert_not_found');
      }

      if (before.status === 'resolved') {
        return { alert: before, alreadyResolved: true };
      }
      ...
      await client.query(
        ...
        VALUES ($1, 'affiliate_alert_resolved', $2, 'affiliate_alert', $3, $4::jsonb, $5::jsonb, $6)
        `,
        [
          actor,
          before.affiliate_id,
          alertId,
          JSON.stringify({ status: before.status, severity: before.severity }),
  ...
  async function listAdminPayouts({
    payoutPeriod = null,
    includeTest = true,
    limit = 100,
  } = {}) {
    const cleanPeriod = payoutPeriod ? cleanPayoutPeriod(payoutPeriod) : null;
    const cleanIncludeTest = cleanBoolean(includeTest, 'includeTest', true);
    const cleanLimit = Math.max(1, Math.min(250, Number(limit) || 100));
    ...
    const result = await pool.query(
      ...
      SELECT
        ...
        a.is_test AS affiliate_is_test,
        a.payout_currency,
        GREATEST(p.amount_due - p.amount_paid, 0)::text AS remaining_owed,
      ...
      LEFT JOIN LATERAL (
        ...
        FROM affiliate_payout_payments pp
        WHERE pp.monthly_payout_id = p.id
      ) payments ON TRUE
      LEFT JOIN LATERAL (
        SELECT COUNT(*)::int AS adjustment_count
        FROM affiliate_adjustments adj
        WHERE adj.monthly_payout_id = p.id
           OR (adj.affiliate_id = p.affiliate_id AND adj.target_period = p.payout_period)
      ) adjustments ON TRUE
      WHERE ($1::date IS NULL OR p.payout_period = $1::date)
        AND ($2::boolean OR a.is_test = FALSE)
      ORDER BY p.payout_period DESC, a.display_name ASC
      LIMIT $3
      `,
      [cleanPeriod, cleanIncludeTest, cleanLimit]
    );
  ...
  function subscriberAlias(affiliateId, originalTransactionId) {
    return crypto
      .createHash('sha256')
  ...
  async function exactSubscriberSnapshot(
    affiliateId,
    appleEnvironment,
    range
  ) {
    const summaryResult = await pool.query(
      ...
      classified AS (
        SELECT
          *,
          ...
          ) AS is_promo_nonrenewal,
          (
            timezone('America/Chicago', attributed_at)::date >= $3::date
            AND timezone('America/Chicago', attributed_at)::date < $4::date
          ) AS joined_in_range,
          COALESCE(
            GREATEST(
              last_observed_at,
      ...
      SELECT
        COUNT(*)::int AS total_referrals,
        COUNT(*) FILTER (WHERE joined_in_range)::int AS range_referrals,

        COUNT(*) FILTER (WHERE is_active)::int AS active_subscribers,
        COUNT(*) FILTER (WHERE current_state = 'promo_active')::int AS promo_active_subscribers,
        ...
        COUNT(*) FILTER (WHERE is_promo_nonrenewal)::int AS promo_nonrenewals,
        COUNT(*) FILTER (
          WHERE is_active AND auto_renew_enabled = FALSE
        )::int AS canceling,
        COUNT(*) FILTER (WHERE has_standard_paid_followup)::int AS converted_total,

        COUNT(*) FILTER (
          WHERE joined_in_range AND is_active
        )::int AS range_active_subscribers,
        COUNT(*) FILTER (
          WHERE joined_in_range AND current_state = 'promo_active'
        )::int AS range_promo_active_subscribers,
        COUNT(*) FILTER (
          WHERE joined_in_range AND current_state = 'paid_renewing'
        )::int AS range_paid_renewing_subscribers,
        COUNT(*) FILTER (
          WHERE joined_in_range AND current_state = 'paid_canceling'
        )::int AS range_paid_canceling_subscribers,
        COUNT(*) FILTER (
          WHERE joined_in_range AND current_state = 'billing_retry'
        )::int AS range_billing_retry,
        COUNT(*) FILTER (
          WHERE joined_in_range AND current_state = 'expired'
        )::int AS range_expired,
        COUNT(*) FILTER (
          WHERE joined_in_range AND current_state = 'pending'
        )::int AS range_pending_state_subscribers,

        COUNT(*) FILTER (
          WHERE joined_in_range AND is_promo_nonrenewal
        )::int AS range_promo_nonrenewals,
        COUNT(*) FILTER (
          WHERE joined_in_range AND is_active AND auto_renew_enabled = FALSE
        )::int AS range_canceling,
        COUNT(*) FILTER (
          WHERE joined_in_range AND has_standard_paid_followup
        )::int AS range_converted_total,
    ...
    const activityResult = await pool.query(
      ...
      SELECT
        ...
        latest_activity_at
      FROM state_classified
      WHERE timezone('America/Chicago', attributed_at)::date >= $3::date
        AND timezone('America/Chicago', attributed_at)::date < $4::date
    ...
    const priceTierResult = await pool.query(
      `
      ...
      latest_paid AS (
        ...
        JOIN LATERAL (
          ...
          WHERE t.original_transaction_id = chain.original_transaction_id
            ...
            AND COALESCE(UPPER(t.offer_type::text), '') NOT IN ('3', 'OFFER_CODE')
          ORDER BY
            t.purchase_date DESC NULLS LAST,
            t.signed_date DESC NULLS LAST,
      ...
      `,
      [affiliateId, appleEnvironment]
    );

    const activePaidPriceTiers = priceTierResult.rows.map(item => {
      const priceMilliunits = Number(item.price_milliunits);
      const amount = Number.isSafeInteger(priceMilliunits) && priceMilliunits >= 0
        ? (priceMilliunits / 1000).toFixed(3).replace(/0$/, '')
        : null;
      return {
        priceMilliunits: Number.isSafeInteger(priceMilliunits) ? priceMilliunits : null,
        amount,
    ...
    const promoActiveSubscribers = Number(row.promo_active_subscribers || 0);
    const paidRenewingSubscribers = Number(row.paid_renewing_subscribers || 0);
    const paidCancelingSubscribers = Number(row.paid_canceling_subscribers || 0);
    const pendingStateSubscribers = Number(row.pending_state_subscribers || 0);
    const billingRetry = Number(row.billing_retry || 0);
    const expired = Number(row.expired || 0);

    const promoNonRenewals = Number(row.promo_nonrenewals || 0);
    const canceling = Number(row.canceling || 0);
    const convertedTotal = Number(row.converted_total || 0);
    const completedPromos = convertedTotal + promoNonRenewals;

    const rangeReferrals = Number(row.range_referrals || 0);
    const rangeActiveSubscribers = Number(row.range_active_subscribers || 0);
    const rangePromoActiveSubscribers = Number(row.range_promo_active_subscribers || 0);
    const rangePaidRenewingSubscribers = Number(row.range_paid_renewing_subscribers || 0);
    const rangePaidCancelingSubscribers = Number(row.range_paid_canceling_subscribers || 0);
    const rangePendingStateSubscribers = Number(row.range_pending_state_subscribers || 0);
    const rangeBillingRetry = Number(row.range_billing_retry || 0);
    const rangeExpired = Number(row.range_expired || 0);

    const rangePromoNonRenewals = Number(row.range_promo_nonrenewals || 0);
    const rangeCanceling = Number(row.range_canceling || 0);
    const rangeConvertedTotal = Number(row.range_converted_total || 0);
    const rangeCompletedPromos = rangeConvertedTotal + rangePromoNonRenewals;

    return {
      hasRows: totalReferrals > 0,
      latestActivityAt: toIsoTimestamp(row.latest_activity_at),
      lifetime: {
        totalReferrals,
        currentSubscribers: activeSubscribers,
        activeSubscribers,

        // Mutually exclusive current-state buckets. Every verified chain is
        // represented by exactly one of these states (including pending).
        promoActiveSubscribers,
        paidRenewingSubscribers,
        paidCancelingSubscribers,
        billingRetry,
        expired,
        pendingStateSubscribers,

        // Backward-compatible aggregate fields retained for internal callers.
        promoSubscribers: promoActiveSubscribers,
        commissionEarningSubscribers:
          paidRenewingSubscribers + paidCancelingSubscribers,
        promoNonRenewals,
        canceling,

        promoRenewalRate: percentOf(convertedTotal, completedPromos),
        promoNonRenewalRate: percentOf(promoNonRenewals, completedPromos),
        paidConversionRate: percentOf(convertedTotal, totalReferrals),
        activeRetention: percentOf(activeSubscribers, totalReferrals),
        cancellationRate: percentOf(canceling, activeSubscribers),
      },
      range: {
        totalReferrals: rangeReferrals,
        currentSubscribers: rangeActiveSubscribers,
        activeSubscribers: rangeActiveSubscribers,

        promoActiveSubscribers: rangePromoActiveSubscribers,
        paidRenewingSubscribers: rangePaidRenewingSubscribers,
        paidCancelingSubscribers: rangePaidCancelingSubscribers,
        billingRetry: rangeBillingRetry,
        expired: rangeExpired,
        pendingStateSubscribers: rangePendingStateSubscribers,

        promoSubscribers: rangePromoActiveSubscribers,
        commissionEarningSubscribers:
          rangePaidRenewingSubscribers + rangePaidCancelingSubscribers,
        promoNonRenewals: rangePromoNonRenewals,
        canceling: rangeCanceling,

        promoRenewalRate: percentOf(rangeConvertedTotal, rangeCompletedPromos),
        promoNonRenewalRate: percentOf(rangePromoNonRenewals, rangeCompletedPromos),
        paidConversionRate: percentOf(rangeConvertedTotal, rangeReferrals),
        activeRetention: percentOf(rangeActiveSubscribers, rangeReferrals),
        cancellationRate: percentOf(rangeCanceling, rangeActiveSubscribers),
      },
      activePaidPriceTiers,
      activity: activityResult.rows.map((item) => ({
        subscriberAlias: `#${subscriberAlias(affiliateId, item.original_transaction_id)}`,
        joinedAt: toIsoTimestamp(item.attributed_at),
        plan: item.product_id || null,
        currentState: item.current_state || 'pending',

        // Retained for compatibility with any internal consumers.
        stage: item.has_standard_paid_followup ? 'commission_earning' : 'promo',
        status: item.status || 'unknown',
        autoRenewEnabled:
          item.auto_renew_enabled == null
            ? null
            : Boolean(item.auto_renew_enabled),

        expiresAt: toIsoTimestamp(item.expires_date),
        lastActivityAt: toIsoTimestamp(item.latest_activity_at),
      })),
    };
  }
  ...
  async function payoutSummary(affiliateId, now, environment = 'production') {
    ...
    const summaryResult = await pool.query(
      ...
      SELECT
        ...
        COALESCE(SUM(eligible_revenue), 0)::text AS lifetime_eligible_revenue
    ...
    return {
      ...
      lifetimeEligibleRevenue: summary.lifetime_eligible_revenue || '0.00',
  ...
  async function getDashboardData(token, rangeValue = 'this_month') {
    ...
    const range = parseAffiliateDashboardRange(rangeValue, now);
    const environment = affiliate.is_test ? 'test' : 'production';
    const appleEnvironment = affiliate.is_test ? 'Sandbox' : 'Production';
    ...
    const aggregateTotalReferrals = lifetimePaidOfferStarts.rows > 0
    ...
    const aggregateOverviewPromoRenewalRate = lifetimeCompletedPromos > 0
    ...
    const aggregatePromoRenewalRate = rangeCompletedPromos > 0
      ? (rangeConversions.value / rangeCompletedPromos) * 100
      : null;
    const aggregatePromoNonRenewalRate = rangeCompletedPromos > 0
      ? (rangePromoNonRenewals / rangeCompletedPromos) * 100
      : null;

    const hasExactSubscribers = exactSubscribers.hasRows;
    const exactLifetime = exactSubscribers.lifetime;
    const exactRange = exactSubscribers.range;
    const exactPriceTierCountsAvailable =
      hasExactSubscribers || Number(aggregateTotalReferrals || 0) === 0;

    const totalReferrals = hasExactSubscribers
      ? exactLifetime.totalReferrals
      : aggregateTotalReferrals;
    const activeSubscribers = hasExactSubscribers
      ? exactLifetime.activeSubscribers
      : currentActiveAccess.value;
    const promoSubscribers = hasExactSubscribers
      ? exactLifetime.promoSubscribers
      : currentPaidOffers.value;
    const commissionEarningSubscribers = hasExactSubscribers
      ? exactLifetime.commissionEarningSubscribers
      : null;

    // Preferred partner-dashboard fields: mutually exclusive current states.
    // For aggregate Apple Analytics fallback data, state-specific paid buckets
    // remain null instead of inventing a split we cannot verify.
    const promoActiveSubscribers = hasExactSubscribers
      ? exactLifetime.promoActiveSubscribers
      : currentPaidOffers.value;
    const paidRenewingSubscribers = hasExactSubscribers
      ? exactLifetime.paidRenewingSubscribers
      : null;
    const paidCancelingSubscribers = hasExactSubscribers
      ? exactLifetime.paidCancelingSubscribers
      : null;
    const pendingStateSubscribers = hasExactSubscribers
      ? exactLifetime.pendingStateSubscribers
      : null;
    const promoNonRenewals = hasExactSubscribers
      ? exactLifetime.promoNonRenewals
      : ((lifetimePromoVoluntaryChurn.rows + lifetimePromoInvoluntaryChurn.rows) > 0
        ? lifetimePromoNonRenewals
        : null);
    const canceling = hasExactSubscribers
      ? exactLifetime.canceling
      : currentCancelingActive.value;
    const expired = hasExactSubscribers
      ? exactLifetime.expired
      : null;
    const billingRetry = hasExactSubscribers
      ? exactLifetime.billingRetry
      : currentBillingRetry.value;
    ...
    const freshestDates = [
      ...
      exactSubscribers.latestActivityAt,
    ].filter(Boolean).sort();

    const currentTerm = termResult.rows[0] || null;
    const subscriberDataSource = hasExactSubscribers
      ? 'verified_subscription_chains'
      : (freshestDates.length ? 'apple_analytics' : 'awaiting_apple_data');
    ...
    return {
      affiliate: {
        ...
        payoutCurrency: affiliate.payout_currency || 'USD',
      ...
      dataFreshness: {
        latestAppleStateDate: freshestDates.length
          ...
          : null,
        latestVerifiedSubscriptionAt:
          exactSubscribers.latestActivityAt,
        status: freshestDates.length
          ? 'apple_data_available'
          : 'awaiting_apple_data',
      ...
      overview: {
        ...
        // Mutually exclusive subscriber-state fields for partner presentation.
        currentSubscribers: activeSubscribers,
        promoActiveSubscribers,
        paidRenewingSubscribers,
        paidCancelingSubscribers,
        pendingStateSubscribers,
        promoRenewalRate: hasExactSubscribers
          ? exactLifetime.promoRenewalRate
          : aggregateOverviewPromoRenewalRate,
        paidConversionRate: hasExactSubscribers
          ? exactLifetime.paidConversionRate
          : null,
        activeRetention: hasExactSubscribers
          ? exactLifetime.activeRetention
          : null,
        cancellationRate: hasExactSubscribers
          ? exactLifetime.cancellationRate
          : null,
        lifetimeEligibleRevenue: payout.lifetimeEligibleRevenue,
        lifetimeCommissionEarned: payout.lifetimeCommissionEarned,
        lifetimePaid: payout.lifetimePaid,
        lastPayment: payout.lastPayment,
      },
      breakdown: {
        subscriberMetrics: {
          totalReferrals,
          newReferrals: hasExactSubscribers
            ? exactRange.totalReferrals
            : (rangePaidOfferStarts.rows > 0 ? rangePaidOfferStarts.value : null),
          activeSubscribers: hasExactSubscribers
            ? exactRange.activeSubscribers
            : rangeActiveAccess.value,
          promoSubscribers: hasExactSubscribers
            ? exactRange.promoSubscribers
            : rangePaidOffers.value,
          activePaidSubscribers: rangePaidPlans.value,
          commissionEarningSubscribers: hasExactSubscribers
            ? exactRange.commissionEarningSubscribers
            : null,
          promoNonRenewals: hasExactSubscribers
            ? exactRange.promoNonRenewals
            : ((rangePromoVoluntaryChurn.rows + rangePromoInvoluntaryChurn.rows) > 0
              ? rangePromoNonRenewals
              : null),
          canceling: hasExactSubscribers
            ? exactRange.canceling
            : rangeCancelingActive.value,
          expired: hasExactSubscribers
            ? exactRange.expired
            : null,
          billingRetry: hasExactSubscribers
            ? exactRange.billingRetry
            : rangeBillingRetry.value,

          // Selected cohort, current state. These buckets are mutually exclusive.
          currentSubscribers: hasExactSubscribers
            ? exactRange.currentSubscribers
            : rangeActiveAccess.value,
          promoActiveSubscribers: hasExactSubscribers
            ? exactRange.promoActiveSubscribers
            : rangePaidOffers.value,
          paidRenewingSubscribers: hasExactSubscribers
            ? exactRange.paidRenewingSubscribers
            : null,
          paidCancelingSubscribers: hasExactSubscribers
            ? exactRange.paidCancelingSubscribers
            : null,
          pendingStateSubscribers: hasExactSubscribers
            ? exactRange.pendingStateSubscribers
            : null,
        },
        performance: {
          promoRenewalRate: hasExactSubscribers
            ? exactRange.promoRenewalRate
            : aggregatePromoRenewalRate,
          promoNonRenewalRate: hasExactSubscribers
            ? exactRange.promoNonRenewalRate
            : aggregatePromoNonRenewalRate,
          paidConversionRate: hasExactSubscribers
            ? exactRange.paidConversionRate
            : null,
          activeRetention: hasExactSubscribers
            ? exactRange.activeRetention
            : null,
          cancellationRate: hasExactSubscribers
            ? exactRange.cancellationRate
            : null,
        },
        payouts: payout,
        anonymousSubscriberActivity:
          exactSubscribers.activity,
      },
      subscriberPricing: {
        source: 'verified_app_store_transactions',
        exactCountsAvailable: exactPriceTierCountsAvailable,
        activePaidPriceTiers: exactPriceTierCountsAvailable
          ? exactSubscribers.activePaidPriceTiers
          : null,
      },
      ...
      reconciliation: {
        ...
        attributedSubscriptionChains: hasExactSubscribers
          ? exactLifetime.totalReferrals
          : null,
      },
    };
  ...
  async function refreshAffiliatePayoutsForPeriod({
    payoutPeriod,
    includeTest = false,
    actor = 'owner_admin',
  } = {}) {
    ...
    for (const affiliate of affiliateResult.rows) {
      try {
        ...
        failures.push({
          affiliateId: affiliate.id,
          customCode: affiliate.normalized_code,
          code: error?.code || 'affiliate_payout_refresh_failed',
          message: error?.message || 'Payout refresh failed.',
        });
      }
    }
  ...
  return Object.freeze({
    createAffiliate,
    listAffiliates,
    setAffiliateOperationalStatus,
    listAffiliateAlerts,
    resolveAffiliateAlert,
    listAdminPayouts,
    findAffiliateByCode,
    recordReferralClick,
    resolvePartnerToken,
    ...
    refreshAffiliatePayoutsForPeriod,
