...
function normalizeDeletionIdentityRow(row) {
    if (!row) return null;
    ...
    return {
        ...
        audience: row.audience,
        subject: row.subject,
        encryptedRefreshToken: row.apple_refresh_token_encrypted,
        accountStatus: row.account_status,
        authVersion: Number(row.auth_version),
        displayName: row.display_name,
    };
    ...
    return Object.freeze({
        ...
        async findDeletionIdentityForUpdate(
            ...
            const result = await query(
                tx,
                `
                    /* account-auth:find-deletion-identity-for-update */
                    SELECT
                        ai.id AS identity_id,
                        ai.account_id,
                        ai.issuer,
                        ai.audience,
                        ai.subject,
                        ai.apple_refresh_token_encrypted,
                        a.status AS account_status,
                        a.auth_version,
                        a.display_name
                    FROM accounts a
                    JOIN account_apple_identities ai
            ...
            return normalizeDeletionIdentityRow(result.rows[0]);
        },
        ...
        async createDeletionRequest(
            ...
            const result = await query(
                tx,
                `
                    /* account-auth:create-deletion-request */
        ...
        async markAccountDeletionPending(
            ...
            const result = await query(
                tx,
                `
                    /* account-auth:mark-deletion-pending */
                    UPDATE accounts
                    SET
        ...
        async revokeAllAccountSessions(
            ...
            await query(
                tx,
                `
                    /* account-auth:revoke-all-account-sessions */
                    UPDATE account_sessions
                    SET
                        revoked_at = COALESCE(revoked_at, $2),
        ...
        async failAccountDeletion(
            ...
            await query(
                tx,
                `
                    /* account-auth:fail-deletion-request */
                    UPDATE account_deletion_requests
                    SET
                        status = 'failed',
                        apple_revocation_status = $4,
                        last_error_code = $5,
            ...
            await query(
                tx,
                `
                    /* account-auth:restore-account-after-deletion-failure */
                    UPDATE accounts
                    SET
                        status = 'active',
                        auth_version = auth_version + 1,
        ...
        async finalizeAccountDeletion(
            ...
            const accountResult = await query(
                tx,
                `
                    /* account-auth:lock-account-for-deletion-finalize */
                    SELECT id
                    FROM accounts
                    WHERE id = $1
                      AND status = 'deletion_pending'
            ...
            await query(
                tx,
                `
                    /* account-auth:release-subscription-ownership */
            ...
            await query(
                tx,
                `
                    /* account-auth:delete-achievement-unlocks */
                    DELETE FROM account_achievement_unlocks
                    WHERE account_id = $1
                `,
                [accountId]
            );
            ...
            await query(
                tx,
                `
                    /* account-auth:delete-daily-challenge-progress */
                    DELETE FROM account_daily_challenge_progress
                    WHERE account_id = $1
                `,
                [accountId]
            );
            ...
            await query(
                tx,
                `
                    /* account-auth:delete-debate-history */
                    DELETE FROM account_debate_history
                    WHERE account_id = $1
                `,
                [accountId]
            );
            ...
            await query(
                tx,
                `
                    /* account-auth:delete-account-sessions */
            ...
            await query(
                tx,
                `
                    /* account-auth:delete-auth-challenges */
            ...
            await query(
                tx,
                `
                    /* account-auth:delete-account-installations */
            ...
            await query(
                tx,
                `
                    /* account-auth:delete-apple-identities */
                    DELETE FROM account_apple_identities
                    WHERE account_id = $1
                `,
                [accountId]
            );
            ...
            const deletedAccount = await query(
                tx,
                `
                    /* account-auth:finalize-account-tombstone */
                    UPDATE accounts
                    SET
                        status = 'deleted',
                        auth_version = auth_version + 1,
            ...
            const requestResult = await query(
                tx,
                `
                    /* account-auth:complete-deletion-request */
                    UPDATE account_deletion_requests
                    SET
                        status = 'completed',
                        apple_revocation_status = $4,
...
export function createAccountAuthService({
    ...
    if (typeof revokeToken !== 'function') {
        fail(
            'invalid_configuration',
            'revokeToken must be a function.',
            { status: 500 }
        );
    }
    ...
    async function signInWithApple(input) {
        try {
            ...
            const challenge = await consumeAppleChallenge({
    ...
    async function deleteAccount(input) {
        ...
        try {
            ...
            deletionContext = await repo.withTransaction(
                async (tx) => {
                    const identity =
                        await repo.findDeletionIdentityForUpdate(
                            tx,
                            {
                                accountId,
                                issuer: credentialIdentity.issuer,
                    ...
                    if (!identity) {
                        fail(
                            ...
                            'The Apple identity for this account is unavailable.',
                            { status: 409 }
                        );
                    }
                    ...
                    const markedPending =
                        await repo.markAccountDeletionPending(
                            tx,
                            {
                                accountId,
                                requestedAt,
                    ...
                    await repo.revokeAllAccountSessions(
                        tx,
                        {
                            accountId,
                            revokedAt: requestedAt,
                            reason:
                                SESSION_REVOCATION_REASON_ACCOUNT_DELETION,
                        }
                    );
            ...
            let appleRevocationStatus = 'manual_required';

            if (deletionContext.identity.encryptedRefreshToken) {
                ...
                try {
                    appleRefreshToken =
                        decryptAppleRefreshToken(
                            deletionContext
                                ...
                                .encryptedRefreshToken,
                            resolvedCryptoConfig,
                            appleRefreshTokenBinding(
                                deletionContext.identity
                            )
                        );
                    ...
                    await repo.withTransaction(async (tx) => {
                        await repo.failAccountDeletion(
                            tx,
                            {
                                accountId,
                                requestId:
                                    deletionContext.requestId,
                                failedAt: nowDate(now),
                                appleRevocationStatus:
                                    'failed',
                                errorCode: failureCode,
                                errorMessage:
                                    'Apple credential revocation failed.',
                            }
                        );
                    });
            ...
            try {
                await repo.withTransaction(async (tx) => {
                    const finalized =
                        await repo.finalizeAccountDeletion(
                            tx,
                            {
                                accountId,
                                requestId:
                ...
                try {
                    ...
                    // Preserve the finalization failure. A pending request can
                    // be recovered manually if the database is unavailable.
                }
            ...
            if (
                ...
                throw error instanceof AppleSignInError
                    ? mapAppleFailure(error)
                    : error;
            }
