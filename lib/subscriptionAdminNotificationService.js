...
function normalizePrimaryChannel(value) {
    return String(value || '').trim().toLowerCase() === 'apns'
        ? 'apns'
        : 'telegram';
}
...
export function createSubscriptionAdminNotificationService({
    ...
    const config = Object.freeze({
        ...
        primaryChannel: normalizePrimaryChannel(
            environment.SUBSCRIPTION_ADMIN_PRIMARY_CHANNEL || 'telegram'
        ),
        ...
        telegramBotToken: cleanText(
            environment.SUBSCRIPTION_ADMIN_TELEGRAM_BOT_TOKEN ||
            environment.TELEGRAM_BOT_TOKEN,
            512
        ),
        telegramChatId: cleanText(
            environment.SUBSCRIPTION_ADMIN_TELEGRAM_CHAT_ID ||
    ...
    async function deliverRow(row) {
        let apnsStatus = hasApnsSelector()
            ? 'not_attempted'
            : 'not_configured';
        let apnsError = null;
        let telegramStatus = hasTelegram()
            ? 'not_attempted'
            : 'not_configured';
        ...
        async function attemptApns() {
            if (!hasApnsSelector()) {
                apnsStatus = 'not_configured';
                return false;
            }
        ...
        if (config.primaryChannel === 'telegram') {
            const telegramDelivered = await attemptTelegram();

            if (telegramDelivered) {
                if (hasApnsSelector()) {
                    apnsStatus = 'not_needed';
                }
            } else {
                await attemptApns();
            }
        } else {
            const apnsDelivered = await attemptApns();

            if (apnsDelivered) {
                if (config.telegramAlways) {
                    const telegramDelivered = await attemptTelegram();
                    if (telegramDelivered) {
            ...
            } else if (config.telegramFallbackEnabled) {
                await attemptTelegram();
            }
        }
    ...
    function start() {
        ...
        if (!hasApnsSelector() && !hasTelegram()) {
            logger.warn(
                '[SubscriptionAdminAlerts] Enabled but no APNs admin target or Telegram destination is configured.'
            );
        }
    ...
    async function enqueueTestNotification({
        ...
        body = 'Telegram is primary. APNs is ready as fallback.',
    } = {}) {
        const dedupeKey =
        ...
        const result = await pool.query(
            ...
            [
                ...
                cleanText(body, 1000) ||
                    'Telegram is primary. APNs is ready as fallback.',
                `${title}\n\n${body}`,
