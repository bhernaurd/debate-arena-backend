...
function buildResult({
    ...
    const wouldDemote = comparePositions(unprotectedAfter, before) < 0;
    const wouldCrossDivision =
        stageIndex(unprotectedAfter) < stageIndex(before);

    let after = unprotectedAfter;
    let protectionApplied = false;
    let protectionConsumed = false;

    // One protected completed debate may still reduce RP inside the current
    // division, but it cannot move the account into any lower division or
    // lower major rank. When a loss would cross that boundary, the account
    // stops at 0 RP in its protected division.
    if (
        !bypassProtection &&
        protectionBefore === 1 &&
        wouldDemote &&
    ...
    const stageComparison = stageIndex(after) - stageIndex(before);
    const promoted = stageComparison > 0;
    const demoted = stageComparison < 0;
