...
function validateAnalysisResult(parsed, { explorationTargets = [] } = {}) {
    ...
    const targetIds = new Set(
        (explorationTargets || []).map((target) => String(target?.id || '')).filter(Boolean)
    );
    const recommendations = Array.isArray(parsed?.recommendations)
        ? parsed.recommendations.filter((item) => {
            ...
            if (item?.philosopher != null && item?.philosopher !== '' &&
                !LIVE_RECOMMENDATION_PHILOSOPHERS.includes(item.philosopher)) {
                return false;
            }
            return true;
