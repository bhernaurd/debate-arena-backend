...
- Recommend exploration, not ideological conversion. Recommendations should broaden underexplored evidence or pressure-test a tension.
  ...
  "recommendations": [
    {
      ...
      "reason": "why this would broaden or pressure-test the next Mirror"
    }
  ]
}
...
function validateAnalysisResult(parsed) {
    const dimensions = Array.isArray(parsed?.dimensions)
        ? parsed.dimensions.filter((item) => MIRROR_DIMENSIONS.includes(item?.dimension)).slice(0, 6)
        : [];
    const recommendations = Array.isArray(parsed?.recommendations)
        ? parsed.recommendations.filter((item) => {
            if (!['philosopher', 'topic', 'tension'].includes(item?.kind)) return false;
            if (item?.philosopher == null || item?.philosopher === '') return true;
            return LIVE_RECOMMENDATION_PHILOSOPHERS.includes(item.philosopher);
        }).slice(0, 3)
        : [];
