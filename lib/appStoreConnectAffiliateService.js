// PARTNER_PRICING_V4: adds cached U.S. subscription pricing from App Store Connect.
...
export function createAppStoreConnectAffiliateService({
  ...
  async function getSubscriptionPricingSummary({
    ...
    const standardPrices = prices.filter(item => !item.preserved);
    const effective = standardPrices
      .filter(item => !item.startDate || item.startDate <= today)
      .sort((left, right) =>
        String(right.startDate || '').localeCompare(String(left.startDate || ''))
      );
    const future = standardPrices
      .filter(item => item.startDate && item.startDate > today)
      .sort((left, right) =>
