...
export function createAppStoreConnectAffiliateService({
  ...
  async function listImports({ existingAffiliates = [], importPreferences = [] } = {}) {
    ...
    for (const [customCode, configurations] of grouped.entries()) {
      ...
      const linkedByCode = affiliatesByCode.get(customCode) || null;
      const linkedByOffer = canonical && canonical.distinctCustomCodesOnOffer === 1
        ? affiliatesByOffer.get(canonical.normalizedOfferName) || null
        : null;
      const linkedAffiliate = linkedByCode || linkedByOffer;
      const needsCanonicalChoice = configurations.length > 1 && !canonical;
      const exactAttributionReady = Boolean(
        canonical && canonical.distinctCustomCodesOnOffer === 1
      );
      const attributionBlockReason = canonical && canonical.distinctCustomCodesOnOffer > 1
        ? `This Apple offer contains ${canonical.distinctCustomCodesOnOffer} different custom creator codes. StoreKit subscription-chain attribution can only identify the shared offer reference, not which custom code was redeemed.`
        : null;
