export function enhanceSubscriptionAdminRevenueHtml(html) {
  ...
  output = output
    ...
    .replace(
      ...
      '<div><h1 id="pageTitle">Overview</h1><div class="sub" id="pageSub">Production subscription health and revenue</div><div class="data-freshness" id="dataFreshness">Subscription data: live · Apple reporting: checking…</div></div>'
    )
    .replace(
      '<h2>Subscriber growth</h2><span id="breakdownChartSummary">Month-end active Pro</span>',
      '<h2>Active Pro growth</h2><span id="breakdownChartSummary">Active Pro over time</span>'
    )
    .replaceAll('Loading subscriber growth...', 'Loading Active Pro growth...')
    .replaceAll('No subscriber growth history yet.', 'No Active Pro history yet.')
    .replaceAll("summary.textContent='Month-end active Pro'", "summary.textContent='Active Pro over time'")
    .replaceAll('aria-label="Subscriber growth by month"', 'aria-label="Active Pro growth by month"')
    .replaceAll("metric('Free period',m.active_trials||0,'Active free-trial subscribers')", "metric('Free trials',m.active_trials||0,'Active free-trial subscribers')")
    .replaceAll("metric('Free period starts',row.trialStarts||0,'Subscribers who started a free trial')", "metric('Free trial starts',row.trialStarts||0,'Subscribers who started a free trial')")
    .replaceAll('<th>Free period starts</th>', '<th>Free trial starts</th>')
    .replaceAll('Current subscriber state. Free period means active free-trial access.', 'Current subscriber state. Free trials are active trial access.')
    .replace(
