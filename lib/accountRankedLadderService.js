...
function rankedAccessWindow(timeZone, checkedAt) {
    const localNow = DateTime.fromJSDate(checkedAt).setZone(timeZone);
    const localUnlock = localNow.startOf('day').set({
        hour: DAILY_UNLOCK_HOUR,
        minute: 0,
        second: 0,
        millisecond: 0,
    });
    const windowStart = localNow < localUnlock
        ? localUnlock.minus({ days: 1 })
        : localUnlock;
    const windowEnd = windowStart.plus({ days: 1 });

    return Object.freeze({
        timeZone,
        challengeDate: windowStart.toISODate(),
        windowStartsAt: windowStart.toJSDate(),
        windowExpiresAt: windowEnd.toJSDate(),
    });
}
...
function requireCanonicalPhilosopher(value) {
    ...
    if (!isRankedPhilosopherID(philosopherId)) {
        fail(
            ...
            'The selected philosopher is not enabled for new Ranked debates.',
            { status: 400 }
        );
    }
...
function normalizeGeneratedTopic(result, expectedPhilosopher, expectedMode, expectedLanguage) {
    ...
    return Object.freeze({
        ...
        displayTopic: requireGeneratedString(result.displayTopic, 'generatedTopic.displayTopic', {
            minimumLength: 12,
            maximumLength: 280,
        }),
...
function validateRequestIdentity(row, requestId, philosopher, debateMode, languageCode) {
...
export function createPostgresAccountRankedLadderRepository(pool) {
    ...
    return Object.freeze({
        ...
        async loadDailyChallenge(client, { challengeDate }) {
            const result = await client.query(
                `
                    /* account-ranked-ladder:load-daily-challenge */
                    SELECT philosopher_id, philosopher_name
        ...
        async hasLadderDebateInWindow(
            client,
            { accountId, windowStartsAt, windowExpiresAt }
        ) {
            const result = await client.query(
                `
                    /* account-ranked-ladder:daily-free-use */
                    SELECT EXISTS (
                        SELECT 1
                        FROM account_ranked_debates
                        WHERE account_id = $1::uuid
            ...
            const result = await client.query(
                `
                    ...
                    SET
                        ...
                        display_topic = $11::text,
                        topic_generated_at = $12::timestamptz,
...
export function createAccountRankedLadderService({
    ...
    async function getProAccess(accountId) {
        try {
    ...
    async function buildRankedAccess({
        client,
        accountId,
        timeZone,
        checkedAt,
        ...
        const window = rankedAccessWindow(timeZone, checkedAt);
        const row = await repo.loadDailyChallenge(client, {
            challengeDate: window.challengeDate,
        });
        const dailyPhilosopherId = row == null
            ? null
            : normalizeText(
                rowValue(row, 'philosopher_id', 'philosopherId'),
                'dailyChallenge.philosopherId',
                100
            ).toLowerCase();
        const dailyPhilosopherName = row == null
            ? null
            : normalizeText(
                rowValue(row, 'philosopher_name', 'philosopherName'),
                'dailyChallenge.philosopherName',
                100
            );
        const isPro = proAccess?.hasProAccess === true;
        const dailyUsed = isPro || dailyPhilosopherId == null
            ? false
            : await repo.hasLadderDebateInWindow(client, {
                accountId,
        ...
        return Object.freeze({
            ...
            challengeDate: window.challengeDate,
            dailyPhilosopherId,
            dailyPhilosopherName,
            freeLadderStartAvailable:
                !isPro && dailyPhilosopherId != null && !dailyUsed,
            windowStartsAt: window.windowStartsAt,
            windowExpiresAt: window.windowExpiresAt,
        });
    }

    function requireLadderAccess(access, philosopher) {
        if (access.isPro) return;

        if (!access.dailyPhilosopherId) {
            fail(
                'ranked_daily_philosopher_unavailable',
                "Today's Daily Philosopher is not available yet.",
                { status: 503, retryable: true }
            );
        }

        if (access.dailyPhilosopherId !== philosopher.id) {
            fail(
                'ranked_daily_philosopher_required',
                "Today's free Ranked debate must use the Daily Philosopher.",
                { status: 403, retryable: false }
            );
        }

        if (!access.freeLadderStartAvailable) {
            fail(
                'ranked_daily_match_used',
                "Today's free Ranked debate is already complete.",
                { status: 409, retryable: false }
            );
        }
    }
    ...
    async function reserve({
        ...
        return repo.withTransaction(async (client) => {
            ...
            if (request) {
                validateRequestIdentity(request, requestId, philosopher, debateMode, languageCode);
            ...
            const rankedAccess = await buildRankedAccess({
                client,
                accountId,
                timeZone,
                checkedAt,
            ...
            requireLadderAccess(rankedAccess, philosopher);
    ...
    async function finalize({
        ...
        return repo.withTransaction(async (client) => {
            ...
            validateRequestIdentity(request, requestId, philosopher, debateMode, generatedTopic.language);
    ...
    async function startLadderDebate({
        ...
        const proAccess = await getProAccess(authorization.accountId);
    ...
    async function getRankedAccess({
        installationId,
        accessToken,
        timezone,
    }) {
        ...
        try {
            const access = await repo.withTransaction((client) =>
                buildRankedAccess({
                    client,
                    accountId: authorization.accountId,
                    timeZone,
            ...
            if (error instanceof AccountRankedLadderError) throw error;
            fail(
                'ranked_access_unavailable',
                'Ranked access could not be refreshed.',
                { status: 503, retryable: true, cause: error }
            );
        }
    }
...
export const accountRankedLadderConstants = Object.freeze({
    ...
    dailyUnlockHour: DAILY_UNLOCK_HOUR,
});
