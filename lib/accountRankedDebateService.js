...
function normalizeDebate(
    ...
    return Object.freeze({
        ...
        languageCode: normalizeLanguageCode(
            rowValue(row, 'language_code', 'languageCode')
        ),
        topic:
            normalizeText(
                rowValue(
        ...
        displayTopic:
            normalizeText(
                rowValue(row, 'display_topic', 'displayTopic') ??
...
function publicDebate(debate) {
    return Object.freeze({
        ...
        debateMode:
            debate.debateMode,
        topic: debate.displayTopic,
        language: debate.languageCode,
        topicFingerprint:
            debate.topicFingerprint,
        topicTheme:
            debate.topicTheme,
...
export function createPostgresAccountRankedDebateRepository(
    ...
    return Object.freeze({
        ...
        async findPlacementCompletedEventForDebate(
            client,
            {
                accountId,
                debateId,
            ...
            const result =
                await client.query(
                    `
                        /* account-ranked-debate:find-placement-completed-event */
                        SELECT
                            final_score_value,
                            after_rank_key,
                            after_division,
                        ...
                        WHERE account_id = $1
                          AND ranked_debate_id = $2
                          AND event_type = 'placement_completed'
...
export function createAccountRankedDebateService({
    ...
    function requireResolvedResultRepository() {
        requireRepositoryMethods([
            'findPlacementProfileForUpdate',
            'findPlacementTrialForUpdate',
            'findPlacementCompletedEventForDebate',
        ]);
    }
    ...
    async function getResolvedDebateResult({
        installationId,
        accessToken,
        debateId,
    }) {
        ...
        // Resolved Ranked history remains available after Pro expires.
        // Starting or continuing Ranked debates still requires current Pro access.
        requireResolvedResultRepository();
            ...
            await repo.withTransaction(
                async (client) => {
                    ...
                    if (
                        ...
                        fail(
                            'ranked_ladder_result_not_available',
                            'Ranked ladder result retrieval is not available in this release.',
                            {
                                status: 409,
                                retryable: false,
                            }
                    ...
                    if (!requestId) {
                        fail(
                            ...
                            'The completed Ranked debate is missing its resolution request identity.',
                            {
                                status: 503,
                                retryable: true,
                            }
                    ...
                    if (
                        ...
                        if (!eventRow) {
                            fail(
                                'ranked_placement_result_unavailable',
                                'The final placement result could not be reconstructed.',
                                {
                                    status: 503,
                                    retryable: true,
                                }
                        ...
                        const placementWeightedScore =
                            normalizeNumber(
                                ...
                                'placementResult.placementWeightedScore',
                                0,
                                10
                            );

                        const startingRankKey =
                            normalizeText(
                                rowValue(
                                    eventRow,
                                    'after_rank_key',
                                ...
                                'placementResult.startingRankKey',
                                50
                            );

                        const startingDivision =
                            normalizeInteger(
                                rowValue(
                                    eventRow,
                                    'after_division',
                                ...
                                'placementResult.startingDivision',
                                1,
                                3
                            );
                        ...
                        const protectionAfter =
                            normalizeInteger(
                                ...
                                'placementResult.protectionAfter',
                                0,
                                1_000
                            );
