...
export function createAccountMirrorService({
    ...
    async function snapshotEvidence(cycleId) {
        const signalsResult = await pool.query(
            `SELECT s.dimension, s.pole, s.confidence, s.stance_strength,
        ...
        const revisionsResult = await pool.query(
            `SELECT r.dimension, r.before_pole, r.after_pole, r.confidence,
                    r.revision_strength, r.before_excerpt, r.after_excerpt,
                    r.revision_summary, e.source_type, e.saved_debate_id,
                    e.debate_completed_at, h.philosopher_name, h.topic
    ...
    async function finalizeAndGenerate({ accountId, cycleId, installationId }) {
            ...
            await withTransaction(async (tx) => {
                await tx.query(
                    `INSERT INTO account_mirror_snapshots (
                        ...
                        blend_status, analysis_json, sonnet_model,
                    ...
                    [cycleId, accountId, cycle.cycleNumber,
                     ...
                     archetype.blendStatus, JSON.stringify(ai.value), ai.model,
    ...
    async function kickGeneration({ accountId, cycleId, installationId }) {
        const key = String(cycleId);
        if (generationLocks.has(key)) return generationLocks.get(key);
        const task = finalizeAndGenerate({ accountId, cycleId, installationId })
            .finally(() => generationLocks.delete(key));
        generationLocks.set(key, task);
        return task;
    }

    async function retryGeneration({ installationId, accessToken }) {
        const auth = await authorize({ installationId, accessToken });
        await requirePro(auth.accountId);
        ...
        void kickGeneration({ accountId: auth.accountId, cycleId: cycle.id, installationId });
    ...
    async function getSnapshot({ installationId, accessToken, cycleNumber }) {
        const auth = await authorize({ installationId, accessToken });
        ...
        return { accountId: auth.accountId, snapshot: await serializeSnapshot(result.rows[0]) };
    }
