export function enhanceSubscriptionAdminLifetimeHtml(html) {
  ...
  output = output
    ...
    .replace(
      ...
      "metric('Paid subscribers',m.active_paid_subscribers||0,'Monthly + annual, excluding trials'),\n        metric('Free trials',m.active_trials||0,'Active free-trial subscribers'),"
    )
    .replace(
    ...
    .replace(
      ...
      "function metricTone(label){ const key=String(label||'').toLowerCase(); if(key.includes('refund')||key.includes('ended')||key.includes('expired')||key.includes('revoked'))return 'tone-negative'; if(key.includes('cancel'))return 'tone-warning'; if(key.includes('free trial')||key.includes('trial start')||key==='trials'||key==='trial')return 'tone-trial'; if(key.includes('apple proceeds')||key.includes('fees + tax')||key.includes('estimated'))return 'tone-estimate'; if(key.includes('gross sales')||key.includes('sales this month')||key.includes('paid subscriber')||key.includes('new paid')||key.includes('free → paid')||key==='monthly'||key==='annual')return 'tone-money'; if(key.includes('active subscription')||key.includes('new subscriber'))return 'tone-primary'; return ''; } function metric(label,value,hint=''){ const tone=metricTone(label); return '<div class=\"metric'+(tone?' '+tone:'')+'\"><div class=\"label\">'+esc(label)+'</div><div class=\"value\">'+esc(value)+'</div><div class=\"hint\">'+esc(hint)+'</div></div>'; }"
    )
