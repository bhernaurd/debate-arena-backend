...
test('mobile subscription admin enhancements stay scoped to mobile layout', () => {
  ...
  assert.match(output, /\.drawer\{display:block;position:fixed;inset:0;width:100vw;max-width:none;height:100dvh;/);
  assert.match(output, /grid-template-columns:none;grid-template-areas:none;gap:0;align-items:initial;/);
  ...
  assert.match(output, /\.detailgrid\{display:grid;grid-template-columns:1fr;gap:10px;margin-top:0;width:100%;/);
  ...
  assert.match(output, /\.event\{display:block;width:100%;margin:0 0 10px;border:1px solid #242832;border-left:3px solid #3a404b;border-radius:12px;/);
