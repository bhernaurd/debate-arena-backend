...
function enhanceDashboardHtml(html) {
  return String(html)
    ...
    .replace(
      `<th>Renewal</th><th>Price</th>`,
      `<th>Next</th><th>Price</th>`
    )
    .replace(
      `'<td>'+esc(r.is_lifetime_pro?'Permanent':(r.auto_renew_enabled===false?'Off':(r.auto_renew_enabled===true?'On':'N/A')))+'</td><td>'+esc(priceMilli(r.price_milliunits,r.currency))`,
      `'<td>'+nextActionHtml(r,r.access_ends_at)+'</td><td>'+esc(priceMilli(r.price_milliunits,r.currency))`
    )
    .replace(
